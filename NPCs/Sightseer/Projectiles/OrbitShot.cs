...
namespace GloryMod.NPCs.Sightseer.Projectiles
{
    internal class OrbitShot : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(texture, Projectile.oldPos[1] - Projectile.position + Projectile.Center - Main.screenPosition, sourceRectangle, Projectile.hostile ? Color.White * opacity : mirageColor * opacity, Projectile.rotation, origin, Projectile.scale * opacity + (float)Math.Sin(timer) * 0.1f, SpriteEffects.None, 0);

            for (int i = 0; i < 4; i++)
            {
                Main.EntitySpriteDraw(texture, Projectile.oldPos[1] - Projectile.position + Projectile.Center - Main.screenPosition + new Vector2(4 - (2 * opacity), 0).RotatedBy(timer + i * MathHelper.TwoPi / 4), sourceRectangle, Projectile.hostile ? new Color(47, 250, 255) * opacity * 0.5f : mirageColor * opacity * 0.5f,
