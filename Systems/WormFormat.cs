...
// WARNING: All code here originates from Examplemod, and has only received minor changes. This is NOT my own 
...
namespace GloryMod.Systems
{
    ...
    public abstract class Worm : ModNPC
    {
        ...
        /// <summary>
        /// Should this worm's weakpoint be its tail?
        /// </summary>
        public bool AttackTail { get; set; }
        ...
        {
            return AttackTail ? (SegmentType == WormSegmentType.Tail ? null : false) : (SegmentType == WormSegmentType.Head ? null : false);
        }
    ...
    public abstract class WormHead : Worm
    {
        ...
        /// <summary>
        /// Whether this worm will use its own behavior, rather than normal worm functionality
        /// </summary>
        
        public bool CustomBehavior { get; set; }

        /// <summary>
        /// Whether this worm will set NPC.ai[2] to a number based on their placement in the worm
        /// Useful for behavior that requires specific segments to be used
        /// </summary>

        public bool NumberBodySegments { get; set; }
        ...
        /// <returns></returns>
        protected int SpawnSegment(IEntitySource source, int type, int latestNPC, float segmentNumber = 0)
        {
            ...
            latestNPC = NPC.NewNPC(source, (int)NPC.Center.X, (int)NPC.Center.Y, type, NPC.whoAmI, 0, latestNPC, segmentNumber);
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (!hasFollower)
                {
                    ...
                    else
                    {
                        ...
                        while (distance > 0)
                        {
                            latestNPC = SpawnSegment(source, BodyType, latestNPC, NumberBodySegments ? (randomWormLength - 2 - distance) : 0);
                            distance--;
                        }
                    }
        ...
        private void HeadAI_Movement(bool collision)
        {
            ...
            else
            {
                ...
                if (!CanFly) HeadAI_Movement_PlayDigSounds(length);
